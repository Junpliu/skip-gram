...
TCMALLOC_LARGE_ALLOC_REPORT_THRESHOLD=500000000000000
...
class word2vec:
    def __init__(self, lr, checkpoint, inputfile, test_only, batch_size, language, vocabulary_size, embedding_dim=200, epoch_num=20,
    ...
    def train(self):
        ...
        self.op = Options(self.inputfile, self.vocabulary_size, self.windows_size, self.neg_sample_num, self.language)
        ...
        self.word_lst = word2idx_dict['word_lst']
            ...
            while self.op.process:
                ...
                if batch_num % 10000 == 0:  #
                    ...
                    if self.language == 'english':
                        ...
                        p1, p2 = self.wordsim()
                        print('epoch,batch={:2d} {}:  pearsonr={:1.3f} {:1.3f} former = {:1.3f} {:1.3f} pair/sec = {:4.2f} loss={:4.3f}'
                              .format(epoch, batch_num, p1, p2, sp1, sp2, (batch_num - batch_new) * self.batch_size / (end - start),
                                 losses.avg))
                    else:
                        print(
                            'epoch,batch={:2d} {}:  pair/sec = {:4.2f} loss={:4.3f}'
            ...
            if self.language == 'english':
                ...
                if p1 > best_p:
                    ...
                    torch.save({'state_dict': self.model.state_dict(),
                                ...
                                'epoch': epoch + 1}, './checkpoints/epoch{}_batch{}_lr{}'.format(epoch, batch_num, self.lr))
            else:
                print('epoch {}\t'
                ...
                if losses.avg > best_loss:
                    ...
                    torch.save({'state_dict': self.model.state_dict(),
                                ...
                                'optimizer': optimizer.state_dict(),
                                'epoch': epoch + 1},
                               './checkpoints/chinese/epoch{}_batch{}_lr{}'.format(epoch, batch_num, self.lr))
...
if __name__ == '__main__':
    ...
    wc = word2vec(inputfile=args.inputfile, checkpoint=args.checkpoint, test_only=args.testonly, language=args.language, lr=args.lr, batch_size=args.batch_size, vocabulary_size=args.vocabsize)
